...
import com.songguoliang.springboot.quartz.QuartzManager;
    ...
    public void initSchedule() {
        ...
        try {
            ...
            if (!quartzManager.checkJobKeyExists(taskName, taskGroup)) {
                ...
                task.setBeanClass("com.songguoliang.springboot.quartz.job.HelloWorldJob");
            ...
            if (!quartzManager.checkJobKeyExists(taskName, taskGroup)) {
                task = new Task();
                task.setTaskId(StringUtil.getRandomUUID());
                task.setBeanClass("com.songguoliang.springboot.quartz.job.Test2Job");
                task.setTaskName(taskName);
            ...
            if (!quartzManager.checkJobKeyExists(taskName, taskGroup)) {
                task = new Task();
                task.setTaskId(StringUtil.getRandomUUID());
                task.setBeanClass("com.songguoliang.springboot.quartz.job.TestJob");
                task.setTaskName(taskName);
