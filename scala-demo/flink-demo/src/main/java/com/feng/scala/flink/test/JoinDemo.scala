...
object JoinDemo {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //获取接口传送的数据
    val dataStream1 = env.readTextFile("C:/work/test/socket1.txt")
    val dataStream2 = env.readTextFile("C:/work/test/socket2.txt")
    val format = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss.SSS")
    //使用样例类StockTransaction封装获取的数据
    val dataStreamMap1 = dataStream1.map(f => {
      println("socket1解析数据：" + f)
      val tokens1 = f.split(",")
    ...
    innerJoinStream.map(f => {
      println("结果：" + f.toString())
    })
