...
public class PPTUtil {
    ...
    public static List<String> converPPTtoImage(String orignalPPTFileName, String targetImageFileDir,
        ...
        List<String> imgNamesList = new ArrayList<>();// PPT转成图片后所有名称集合
        ...
        try {
            ...
            boolean isOld = false, isNew = false;
            try {
                String __name = orignalPPTFileName.toLowerCase(Locale.getDefault());
                if (__name.endsWith(".ppt")) {
                    slideShow = new HSLFSlideShow(orignalPPTFileInputStream);
                    isOld = true;
                }
                else if (__name.endsWith(".pptx")) {
                    slideShow = new XMLSlideShow(orignalPPTFileInputStream);
                    isNew = true;
                }
            } catch (IOException e) {
                e.printStackTrace();
            ...
            System.err.println("isOld => " + isOld + "; isNew => " + isNew);
            if (null != slideShow) {
                // 获取PPT每页的大小（宽和高度）
                Dimension onePPTPageSize = slideShow.getPageSize();
                // 获得PPT文件中的所有的PPT页面（获得每一张幻灯片）,并转为一张张的播放片
                List<Slide<?, ?>> pptPageSlideList = slideShow.getSlides();
                if (isOld) {
//                    List<HSLFSlide> __list = slideShow.getSlides();
                    // 下面循环的主要功能是实现对PPT文件中的每一张幻灯片进行转换和操作
                    for (int i = 0; i < pptPageSlideList.size(); i++) {
                        HSLFSlide slide = (HSLFSlide)pptPageSlideList.get(i);
                ...
                if (isNew) {
                    // 获得PPT文件中的所有的PPT页面（获得每一张幻灯片）,并转为一张张的播放片
//                    List<XSLFSlide> pptPageSlideList = slideShow.getSlides();
                    // 下面循环的主要功能是实现对PPT文件中的每一张幻灯片进行转换和操作
                    for (int i = 0; i < pptPageSlideList.size(); i++) {
                        // 这几个循环只要是设置字体为宋体，防止中文乱码，
                        XSLFSlide slide = (XSLFSlide)pptPageSlideList.get(i);
...
//        System.err.println("ppt文件导出图片");
//        List<String> result = converPPTtoImage(path + "tianshiyuemo.ppt", path + File.separator + "ppt-images" + File.separator, "jpg", 8);
//        for(String s:result){
//            System.out.println(s);
//        }

        System.err.println("pptx文件导出图片");

        List<String> result = converPPTtoImage(path + "tianshiyuemo.pptx", path + File.separator + "pptx-images" + File.separator, "jpg", 8);
        for(String s:result){
            System.out.println(s);
        }
    }
